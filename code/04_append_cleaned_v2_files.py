...
CLEANED_DIR = BASE_DIR / "cleaned_v2"
MERGED_DIR = BASE_DIR / "merged_v2"
...
def write_log(main_info: dict[str, Any], extended_info: dict[str, Any]) -> None:
    lines: list[str] = []
    lines.append("NIA cleaned_v2 파일 append 로그")
    lines.append("=" * 96)
    ...
    for title, info in [
        ...
        lines.append("missing standardized columns added artificially:")
        any_missing = False
        for item in info["year_logs"]:
...
def main() -> None:
    ...
    print(
        f"[main_v2] rows={main_info['actual_rows']}, years={main_info['year_distribution']}, "
        f"row_check={main_info['expected_rows'] == main_info['actual_rows']}"
    )
    print(
        f"[extended_v2] rows={extended_info['actual_rows']}, years={extended_info['year_distribution']}, "
